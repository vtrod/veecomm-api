...
type ProductQuery struct {
	...
	withAvaliations *AvaliationQuery
...
// QueryAvaliations chains the current query on the "avaliations" edge.
func (pq *ProductQuery) QueryAvaliations() *AvaliationQuery {
	query := (&AvaliationClient{config: pq.config}).Query()
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withAvaliations: pq.withAvaliations.Clone(),
...
// WithAvaliations tells the query-builder to eager-load the nodes that are connected to
// the "avaliations" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithAvaliations(opts ...func(*AvaliationQuery)) *ProductQuery {
	query := (&AvaliationClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withAvaliations = query
	return pq
}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := pq.withAvaliations; query != nil {
		if err := pq.loadAvaliations(ctx, query, nodes,
			func(n *Product) { n.Edges.Avaliations = []*Avaliation{} },
			func(n *Product, e *Avaliation) { n.Edges.Avaliations = append(n.Edges.Avaliations, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *ProductQuery) loadAvaliations(ctx context.Context, query *AvaliationQuery, nodes []*Product, init func(*Product), assign func(*Product, *Avaliation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(avaliation.FieldProductID)
	}
	query.Where(predicate.Avaliation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.AvaliationsColumn), fks...))
	}))
