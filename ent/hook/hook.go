...
// The AvaliationFunc type is an adapter to allow the use of ordinary
// function as Avaliation mutator.
type AvaliationFunc func(context.Context, *ent.AvaliationMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AvaliationFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.AvaliationMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AvaliationMutation", m)
}
