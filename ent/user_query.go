...
func (uq *UserQuery) QueryAvaliations() *AvaliationQuery {
	query := (&AvaliationClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withAvaliations: uq.withAvaliations.Clone(),
...
func (uq *UserQuery) WithAvaliations(opts ...func(*AvaliationQuery)) *UserQuery {
	query := (&AvaliationClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withAvaliations = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withAvaliations; query != nil {
		if err := uq.loadAvaliations(ctx, query, nodes,
			func(n *User) { n.Edges.Avaliations = []*Avaliation{} },
			func(n *User, e *Avaliation) { n.Edges.Avaliations = append(n.Edges.Avaliations, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadAvaliations(ctx context.Context, query *AvaliationQuery, nodes []*User, init func(*User), assign func(*User, *Avaliation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(avaliation.FieldUserID)
	}
	query.Where(predicate.Avaliation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AvaliationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
