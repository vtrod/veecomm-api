...
func (uc *UserCreate) AddAddressIDs(ids ...string) *UserCreate {
...
func (uc *UserCreate) AddAvaliationIDs(ids ...string) *UserCreate {
	uc.mutation.AddAvaliationIDs(ids...)
	return uc
}

// AddAvaliations adds the "avaliations" edges to the Avaliation entity.
func (uc *UserCreate) AddAvaliations(a ...*Avaliation) *UserCreate {
	ids := make([]string, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
	return uc.AddAvaliationIDs(ids...)
}

// SetCartID sets the "cart" edge to the Cart entity by ID.
func (uc *UserCreate) SetCartID(id string) *UserCreate {
	uc.mutation.SetCartID(id)
	return uc
}

// SetNillableCartID sets the "cart" edge to the Cart entity by ID if the given value is not nil.
func (uc *UserCreate) SetNillableCartID(id *string) *UserCreate {
	if id != nil {
		uc = uc.SetCartID(*id)
	}
	return uc
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.AvaliationsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
